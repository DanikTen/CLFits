...
def test_search_by_key(tmp_path: Path):
    ...
    assert "OBSERVER" not in result.output
...
def test_search_case_sensitive(tmp_path: Path):
    ...
    assert result.exit_code == 0
    assert "No matching keywords found" in result.output
    # This should succeed
    ...
    assert "OBJECT" in result_correct_case.output
...
def test_search_no_filter_error(tmp_path: Path):
    ...
    assert "Error: At least one of --key or --value must be provided" in result.output
...
def test_search_with_none_value(tmp_path: Path):
    ...
    assert result.exit_code == 0
    assert "NONE_KW" not in result.output
